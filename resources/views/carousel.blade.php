...
@section('content')
    <div class="max-w-[1440px] mx-auto grid grid-cols-1 xl:grid-cols-12 gap-8 pb-12">
        ...
        <aside class="xl:col-span-5 2xl:col-span-4">
            <div
                class="xl:sticky xl:top-24 rounded-3xl border border-outline-variant/70 bg-surface-container-lowest/95 shadow-[0_18px_60px_rgba(0,0,0,0.07)] overflow-hidden backdrop-blur">
                <div class="relative p-6 border-b border-outline-variant/60 overflow-hidden">
                    ...
                    <div class="relative flex items-start justify-between gap-4">
                        <div>
                            <div
                                class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary/10 text-primary text-label-sm font-bold mb-3">
                            ...
                            <p class="mt-2 text-body-md text-on-surface-variant">
                                Upload storyboard PDF, pilih nomor reel, tambahkan referensi karakter, lalu render output
                                visual.
                            </p>
                        </div>

                        <div
                            class="hidden sm:flex w-12 h-12 rounded-2xl bg-primary-container text-on-primary-container items-center justify-center shadow-lg shadow-primary/10">
                            <span class="material-symbols-outlined"
                                style="font-variation-settings:'FILL' 1;">movie_creation</span>
                        </div>
                    </div>

                    <div class="relative mt-5 grid grid-cols-3 gap-3">
                        <div
                            class="rounded-2xl border border-outline-variant/60 bg-surface-container-lowest/80 px-3 py-3 shadow-sm">
                ...
                <div class="p-6 space-y-7">
                    ...
                    <section>
                        <div class="flex items-center justify-between mb-3">
                            <h2 class="font-headline-md text-body-md font-bold text-on-surface flex items-center gap-2">
                                <span
                                    class="w-7 h-7 rounded-full bg-primary text-on-primary flex items-center justify-center text-label-sm font-bold">1</span>
                                Upload PDF Storyboard
                            </h2>

                            <span id="pdfBadge"
                                class="hidden px-3 py-1 rounded-full bg-primary-container text-on-primary-container text-label-sm font-bold">
                                PDF Siap
                            </span>
                        </div>
                        ...
                        <button id="pdfDropZone" type="button"
                            ...
                            <div
                                class="absolute -right-16 -top-16 h-40 w-40 rounded-full bg-primary-container/20 blur-3xl transition-transform group-hover:scale-125">
                            </div>

                            <div class="relative flex h-full items-center gap-4">
                                <div id="pdfIconWrap"
                                    class="flex h-16 w-16 shrink-0 items-center justify-center rounded-3xl bg-surface-container-lowest text-primary shadow-sm border border-outline-variant/50">
                                ...
                                <div class="min-w-0 flex-1">
                                    ...
                                    <p id="pdfFileName" class="mt-1 hidden truncate text-label-sm font-bold text-primary">
                                    </p>
                        ...
                        <!-- Storyboard Analysis Preview Box (4-Part Schema) -->
                        <div id="storyboardAnalysisBox" class="mt-4 hidden rounded-2xl border border-primary/20 bg-primary/5 p-4 transition-all space-y-3">
                            <div class="flex items-center justify-between gap-2 border-b border-primary/10 pb-2">
                                <div class="flex items-center gap-2">
                                    <span class="material-symbols-outlined text-primary text-[20px]">movie</span>
                                    <span class="font-bold text-label-md text-on-surface">Hasil Analisis Carousel</span>
                                </div>
                                <div class="flex items-center gap-2">
                                    <button id="editAnalysisBtn" type="button" title="Edit hasil analisis"
                                        class="flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-bold bg-tertiary/15 text-tertiary hover:bg-tertiary/25 transition-colors cursor-pointer">
                                        <span class="material-symbols-outlined text-[15px]">edit</span>
                                        Edit
                                    </button>
                                    <button id="saveAnalysisBtn" type="button" title="Simpan perubahan"
                                        class="hidden flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-bold bg-primary text-on-primary hover:bg-primary/85 transition-colors cursor-pointer">
                                        <span class="material-symbols-outlined text-[15px]">check</span>
                                        Simpan
                                    </button>
                                    <button id="cancelAnalysisBtn" type="button" title="Batalkan perubahan"
                                        class="hidden flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-bold bg-error/15 text-error hover:bg-error/25 transition-colors cursor-pointer">
                                        <span class="material-symbols-outlined text-[15px]">close</span>
                                        Batal
                                    </button>
                                    <span id="analysisReelBadge" class="px-2.5 py-0.5 rounded-full bg-primary text-on-primary text-[11px] font-extrabold">Reel 1</span>
                                </div>
                            </div>

                            <!-- 1. Visual (Instruksi Gambar) -->
                            <div class="rounded-xl border border-outline-variant/60 bg-surface-container-lowest p-3">
                                <div class="flex items-center gap-1.5 text-[11px] font-bold text-primary mb-1">
                                    <span class="material-symbols-outlined text-[15px]">image</span>
                                    Visual (Instruksi Gambar)
                                </div>
                                <p id="extractedVisualPreview" class="text-body-sm font-medium text-on-surface leading-relaxed italic whitespace-pre-line">
                                    -
                                </p>
                                <textarea id="editVisualInput" rows="3"
                                    class="hidden w-full mt-1 p-2 rounded-lg border border-primary/30 bg-surface text-body-sm text-on-surface font-medium leading-relaxed focus:outline-none focus:ring-2 focus:ring-primary/40 resize-y transition-all"
                                    placeholder="Masukkan instruksi visual..."></textarea>
                            </div>

                            <!-- 2. Teks Utama (Copywriting) -->
                            <div class="rounded-xl border border-outline-variant/60 bg-surface-container-lowest p-3">
                                <div class="flex items-center gap-1.5 text-[11px] font-bold text-secondary mb-1">
                                    <span class="material-symbols-outlined text-[15px]">format_quote</span>
                                    Teks Utama (Copywriting)
                                </div>
                                <p id="extractedCopywritingPreview" class="text-body-sm font-semibold text-on-surface leading-relaxed whitespace-pre-line">
                                    -
                                </p>
                                <textarea id="editCopywritingInput" rows="2"
                                    class="hidden w-full mt-1 p-2 rounded-lg border border-secondary/30 bg-surface text-body-sm text-on-surface font-semibold leading-relaxed focus:outline-none focus:ring-2 focus:ring-secondary/40 resize-y transition-all"
                                    placeholder="Masukkan teks copywriting..."></textarea>
                            </div>

                            <!-- 3. Tujuan (Opsional) -->
                            <div id="tujuanContainer" class="rounded-xl border border-outline-variant/60 bg-surface-container-lowest p-3">
                                <div class="flex items-center gap-1.5 text-[11px] font-bold text-on-surface-variant mb-1">
                                    <span class="material-symbols-outlined text-[15px]">flag</span>
                                    Tujuan (Opsional)
                                </div>
                                <p id="extractedTujuanPreview" class="text-body-sm font-medium text-on-surface-variant leading-relaxed whitespace-pre-line">
                                    -
                                </p>
                                <textarea id="editTujuanInput" rows="2"
                                    class="hidden w-full mt-1 p-2 rounded-lg border border-outline-variant/30 bg-surface text-body-sm text-on-surface-variant font-medium leading-relaxed focus:outline-none focus:ring-2 focus:ring-outline-variant/40 resize-y transition-all"
                                    placeholder="Masukkan tujuan (opsional)..."></textarea>
                            </div>
                        </div>

                        <!-- Raw PDF Text Preview -->
                        <div id="rawTextContainer" class="mt-4 hidden rounded-2xl border border-outline-variant/60 bg-surface-container-lowest p-4 transition-all">
                            <div class="flex items-center gap-2 border-b border-outline-variant/50 pb-2 mb-3">
                                <span class="material-symbols-outlined text-[20px] text-on-surface-variant">subject</span>
                                <span class="font-bold text-label-md text-on-surface">Raw Teks PDF Ekstraksi</span>
                            </div>
                            <div class="max-h-48 overflow-y-auto bg-surface-container-low rounded-xl p-3 border border-outline-variant/30">
                                <pre id="rawTextContent" class="text-[11px] font-mono text-on-surface-variant whitespace-pre-wrap leading-relaxed break-words">-</pre>
                            </div>
                        </div>
                    </section>
                    ...
                    <section>
                        <div class="flex items-center justify-between mb-3">
                            <h2 class="font-headline-md text-body-md font-bold text-on-surface flex items-center gap-2">
                                <span
                                    class="w-7 h-7 rounded-full bg-primary text-on-primary flex items-center justify-center text-label-sm font-bold">2</span>
                                Foto Referensi
                            </h2>

                            <button id="clearReferenceBtn" type="button"
                                class="hidden text-label-sm font-bold text-error hover:underline">
                                Hapus semua
                            </button>
                        </div>
                        ...
                        <button id="referenceDropZone" type="button"
                            ...
                            <div id="referenceEmptyState"
                                class="flex min-h-[140px] flex-col items-center justify-center text-center p-5">
                                <span
                                    class="w-14 h-14 rounded-2xl bg-primary-container text-on-primary-container flex items-center justify-center mb-3 group-hover:scale-105 transition-transform">
                                    <span class="material-symbols-outlined text-3xl">add_photo_alternate</span>
                                </span>
                                <span class="font-bold text-on-surface">Klik atau drag foto referensi</span>
                                <span class="text-label-sm text-on-surface-variant mt-1">Bisa upload lebih dari satu foto
                                    karakter/model.</span>
                            </div>

                            <div id="referenceLoadedState"
                                class="hidden min-h-[140px] rounded-2xl border border-[#00875A]/30 bg-[#EEF9F2] p-5 text-center">
                                <div
                                    class="mx-auto mb-3 flex h-11 w-11 items-center justify-center rounded-full bg-white text-[#00875A] shadow-sm">
                                    <span class="material-symbols-outlined"
                                        style="font-variation-settings:'FILL' 1;">check_circle</span>
                                </div>

                                <p class="font-extrabold text-[#007A5A]">Karakter Dimuat ✓</p>
                                <p id="referenceLoadedText" class="mt-1 text-label-sm text-on-surface-variant">0 foto
                                    referensi siap dipakai</p>
                    ...
                    <section>
                        <div class="flex items-center justify-between mb-3">
                            <h2 class="font-headline-md text-body-md font-bold text-on-surface flex items-center gap-2">
                                <span
                                    class="w-7 h-7 rounded-full bg-primary text-on-primary flex items-center justify-center text-label-sm font-bold">3</span>
                                Pilih Nomor Reel
                            </h2>

                            <span id="reelCountBadge"
                                class="px-3 py-1 rounded-full bg-surface-container-low text-on-surface-variant text-label-sm font-bold">
                                Upload PDF dulu
                            </span>
                        </div>

                        <div class="relative">
                            <select id="reelSelect" disabled
                    ...
                    <section>
                        <div class="flex items-center justify-between mb-3">
                            <h2 class="font-headline-md text-body-md font-bold text-on-surface flex items-center gap-2">
                                <span
                                    class="w-7 h-7 rounded-full bg-primary text-on-primary flex items-center justify-center text-label-sm font-bold">4</span>
                                Kondisi Tambahan
                            </h2>
                        ...
                        <div class="relative">
                            <textarea id="extraPrompt" maxlength="700"
                                ...
                                placeholder="Contoh: gaya cinematic, warna hangat, karakter terlihat natural, background clean, detail produk tetap jelas..."></textarea>
                        </div>
                    </section>
                    ...
                    <section>
                        <div class="flex items-center justify-between mb-3">
                            <h2 class="font-headline-md text-body-md font-bold text-on-surface flex items-center gap-2">
                                <span
                                    class="w-7 h-7 rounded-full bg-primary text-on-primary flex items-center justify-center text-label-sm font-bold">5</span>
                                Aspect Ratio
                            </h2>
                        ...
                        <div id="aspectRatioGroup"
                            class="grid grid-cols-5 gap-1 p-1 bg-surface-container-low rounded-2xl border border-outline-variant/50">
                            <button type="button" data-ratio="16:9" data-label="Landscape"
                                class="aspect-btn rounded-xl py-2.5 px-2 bg-primary-container text-on-primary-container shadow-sm font-label-md text-label-md transition-all">16:9</button>
                            <button type="button" data-ratio="9:16" data-label="Story"
                                class="aspect-btn rounded-xl py-2.5 px-2 text-on-surface-variant hover:bg-surface-container-high font-label-md text-label-md transition-all">9:16</button>
                            <button type="button" data-ratio="1:1" data-label="Square"
                                class="aspect-btn rounded-xl py-2.5 px-2 text-on-surface-variant hover:bg-surface-container-high font-label-md text-label-md transition-all">1:1</button>
                            <button type="button" data-ratio="4:5" data-label="Portrait"
                                class="aspect-btn rounded-xl py-2.5 px-2 text-on-surface-variant hover:bg-surface-container-high font-label-md text-label-md transition-all">4:5</button>
                            <button type="button" data-ratio="3:2" data-label="Photo"
                                class="aspect-btn rounded-xl py-2.5 px-2 text-on-surface-variant hover:bg-surface-container-high font-label-md text-label-md transition-all">3:2</button>
                        </div>
                    </section>
                    ...
                    <button id="renderBtn"
                        ...
                        <span id="renderBtnIcon" class="material-symbols-outlined relative"
        ...
        <main class="xl:col-span-7 2xl:col-span-8 min-w-0">
            <div class="mb-6 flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
                ...
                <div class="flex flex-wrap gap-2">
                    <span id="summaryReel"
                        class="px-3 py-2 rounded-xl bg-surface-container-lowest border border-outline-variant text-label-sm font-bold text-on-surface-variant">Reel
                        -</span>
                    <span id="summaryRatio"
                        class="px-3 py-2 rounded-xl bg-surface-container-lowest border border-outline-variant text-label-sm font-bold text-on-surface-variant">Rasio
                        16:9</span>
                </div>
            </div>

            <section
                class="rounded-3xl border border-outline-variant/70 bg-surface-container-lowest/80 shadow-[0_18px_60px_rgba(0,0,0,0.06)] p-4 sm:p-6 min-h-[560px] backdrop-blur">
                <div id="renderGrid" class="grid min-h-[510px] grid-cols-1 gap-5 md:grid-cols-2">
                    <div id="emptyRenderState"
                        class="md:col-span-2 min-h-[510px] rounded-3xl border-2 border-dashed border-outline-variant/80 bg-gradient-to-br from-surface-container-lowest via-surface-container-low to-primary-container/10 flex flex-col items-center justify-center text-center p-8 overflow-hidden relative">
                        <div class="absolute -top-16 -left-16 w-48 h-48 rounded-full bg-primary-container/20 blur-3xl">
                        </div>
                        <div class="absolute -bottom-20 -right-20 w-56 h-56 rounded-full bg-primary/10 blur-3xl"></div>

                        <div
                            class="relative w-20 h-20 rounded-3xl bg-surface-container-lowest border border-outline-variant shadow-lg flex items-center justify-center mb-5">
                        ...
                        <p class="relative text-body-md text-on-surface-variant max-w-md leading-relaxed">
                            Upload PDF storyboard, pilih reel, tambahkan referensi bila perlu, lalu klik Mulai Render
                            Otomatis.
                        </p>

                        <div class="relative mt-6 flex flex-wrap justify-center gap-2">
                            <span
                                class="px-3 py-1.5 rounded-full bg-white/80 border border-outline-variant text-label-sm text-on-surface-variant">PDF
                                storyboard</span>
                            <span
                                class="px-3 py-1.5 rounded-full bg-white/80 border border-outline-variant text-label-sm text-on-surface-variant">Dropdown
                                reel</span>
                            <span
                                class="px-3 py-1.5 rounded-full bg-white/80 border border-outline-variant text-label-sm text-on-surface-variant">Tanpa
                                contoh palsu</span>
                        </div>
                    </div>
                </div>
    ...
    <!-- Toast -->
    <div id="toast"
        class="pointer-events-none fixed bottom-6 right-6 z-[90] hidden max-w-sm rounded-2xl border border-outline-variant/40 bg-surface-container-lowest p-4 shadow-[0px_18px_55px_rgba(0,0,0,0.18)]">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            const MAX_REFERENCE_FILES = 5; // Disesuaikan dengan batas maksimal backend
            ...
            const PROCESS_ENDPOINT = @json(url('/carousel/process'));
            ...
            const elements = {
                ...
                storyboardAnalysisBox: document.getElementById('storyboardAnalysisBox'),
                analysisReelBadge: document.getElementById('analysisReelBadge'),
                extractedVisualPreview: document.getElementById('extractedVisualPreview'),
                extractedCopywritingPreview: document.getElementById('extractedCopywritingPreview'),
                extractedTujuanPreview: document.getElementById('extractedTujuanPreview'),
                tujuanContainer: document.getElementById('tujuanContainer'),

                editAnalysisBtn: document.getElementById('editAnalysisBtn'),
                saveAnalysisBtn: document.getElementById('saveAnalysisBtn'),
                cancelAnalysisBtn: document.getElementById('cancelAnalysisBtn'),
                editVisualInput: document.getElementById('editVisualInput'),
                editCopywritingInput: document.getElementById('editCopywritingInput'),
                editTujuanInput: document.getElementById('editTujuanInput'),
            ...
            elements.reelSelect.addEventListener('change', () => {
                updateStatusSummary();
                updateAnalysisBoxForSelectedReel();
            });
            ...
            async function handlePdfFile(file) {
                ...
                state.processedReels = [];
                state.isProcessingPdf = true;

                elements.pdfUploadText.textContent = 'Memproses & Menganalisis Storyboard...';
                elements.pdfFileName.textContent = file.name;
                elements.pdfFileName.classList.remove('hidden');
                elements.pdfInfo.textContent = 'Menganalisis adegan visual dari PDF storyboard dengan AI...';
                elements.pdfBadge.textContent = 'Memproses...';
                elements.pdfBadge.classList.remove('hidden');
                elements.pdfDropZone.classList.add('is-ready');

                resetReelDropdown('Memproses storyboard...');

                try {
                    const formData = new FormData();
                    const csrfToken = document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || '';
                    formData.append('_token', csrfToken);
                    formData.append('storyboard_pdf', file);
                    ...
                    if (response.ok && data.success && data.status === 'processing') {
                        // Tampilkan Raw Text
                        if (data.raw_text) {
                            ...
                            if (rawTextContainer && rawTextContent) {
                                rawTextContent.textContent = data.raw_text;
                                rawTextContainer.classList.remove('hidden');
                            }
                        }

                        elements.pdfUploadText.textContent = 'AI sedang mengekstrak teks... (membutuhkan waktu)';
                        elements.pdfBadge.textContent = 'Menunggu AI...';

                        // Polling Text-Generation
                        const finalData = await new Promise((resolve, reject) => {
                            const interval = setInterval(async () => {
                                try {
                                    const statusRes = await fetch(`/carousel/process-status/${data.prompt_id}`);
                                    ...
                                    if (statusData.status === 'done') {
                                        ...
                                        clearInterval(interval);
                                        reject(new Error(statusData.message || 'Error saat polling AI.'));
                                    }
                        ...
                        state.processedReels = finalData.reels;
                        state.detectedReelCount = finalData.total_reels || finalData.reels.length;

                        populateReelDropdownFromProcessed(state.processedReels);

                        elements.pdfUploadText.textContent = 'Storyboard Berhasil Diproses ✓';
                        elements.pdfInfo.textContent = `${state.detectedReelCount} reel terdeteksi & adegan visual diekstrak AI.`;
                        elements.pdfBadge.textContent = 'Storyboard Diproses ✓';

                        showFormMessage(`Storyboard berhasil diproses. ${state.detectedReelCount} reel siap dirender!`, 'success');
                        showToast('Storyboard Diproses', `${state.detectedReelCount} reel terdeteksi & adegan visual diekstrak.`, 'success');

                    } else if (response.ok && data.success && data.reels && data.reels.length > 0) {
                        state.processedReels = data.reels;
                        state.detectedReelCount = data.total_reels || data.reels.length;
                        ...
                        throw new Error(data.message || 'Gagal memproses PDF Storyboard.');
                    }
                } catch (err) {
                    console.warn('Backend process error, client fallback scan:', err);

                    const detection = await detectPdfReelCount(file);
                    state.detectedReelCount = Math.max(1, detection.reelCount || 1);
                    state.processedReels = Array.from({ length: state.detectedReelCount }, (_, i) => ({
                        reel: i + 1,
                        title: `Reel ${i + 1}`,
                        prompt: `Cinematic scene description for reel ${i + 1}`
                    }));

                    populateReelDropdownFromProcessed(state.processedReels);

                    elements.pdfUploadText.textContent = 'PDF Berhasil Dimuat';
                    elements.pdfInfo.textContent = `${state.detectedReelCount} reel terdeteksi. Silakan pilih reel untuk render.`;
                    elements.pdfBadge.textContent = 'PDF Siap';
                    elements.pdfBadge.classList.remove('hidden');

                    showFormMessage(`PDF dimuat. ${state.detectedReelCount} pilihan reel siap.`, 'info');
                    showToast('PDF Dimuat', `${state.detectedReelCount} reel disiapkan.`, 'info');
                } finally {
            ...
            function populateReelDropdownFromProcessed(reels) {
                elements.reelSelect.innerHTML = '';

                // Baris pertama: Header kolom (tidak bisa dipilih)
                const headerOpt = document.createElement('option');
                headerOpt.value = '';
                headerOpt.textContent = 'Reel | Visual (Instruksi Gambar) | Copywriting | Tujuan';
                headerOpt.disabled = true;
                headerOpt.style.fontWeight = 'bold';
                headerOpt.style.color = '#888';
                elements.reelSelect.appendChild(headerOpt);

                reels.forEach((item) => {
                    const opt = document.createElement('option');
                    opt.value = String(item.reel);
                    opt.textContent = item.title || `Reel ${item.reel}`;
                    elements.reelSelect.appendChild(opt);
                });

                elements.reelSelect.disabled = false;
                elements.reelCountBadge.textContent = `${reels.length} reel tersedia`;
                elements.reelSelect.value = String(reels[0]?.reel || 1);
                updateAnalysisBoxForSelectedReel();
                updateStatusSummary();
            }

            function updateAnalysisBoxForSelectedReel() {
                const selectedVal = Number.parseInt(elements.reelSelect.value, 10) || 1;
                const found = state.processedReels.find(r => r.reel === selectedVal) || state.processedReels[0];
                if (found && elements.storyboardAnalysisBox) {
                    elements.storyboardAnalysisBox.classList.remove('hidden');
                    elements.analysisReelBadge.textContent = found.title || `Reel ${found.reel}`;

                    if (elements.extractedVisualPreview) {
                        elements.extractedVisualPreview.textContent = found.visual || found.prompt || `Adegan visual untuk Reel ${found.reel}`;
                    }
                    if (elements.extractedCopywritingPreview) {
                        elements.extractedCopywritingPreview.textContent = found.copywriting || '-';
                    }
                    if (elements.extractedTujuanPreview) {
                        elements.extractedTujuanPreview.textContent = found.tujuan || '-';
                    }

                    // Keluar dari mode edit saat ganti reel
                    exitAnalysisEditMode();
                }
            }

            // =============================================
            // Edit Hasil Analisis Carousel
            // =============================================
            function enterAnalysisEditMode() {
                const selectedVal = Number.parseInt(elements.reelSelect.value, 10) || 1;
                const found = state.processedReels.find(r => r.reel === selectedVal);
                if (!found) return;

                // Isi textarea dengan value terkini
                elements.editVisualInput.value = found.visual || found.prompt || '';
                elements.editCopywritingInput.value = found.copywriting || '';
                elements.editTujuanInput.value = found.tujuan || '';

                // Tampilkan textarea, sembunyikan preview
                elements.extractedVisualPreview.classList.add('hidden');
                elements.extractedCopywritingPreview.classList.add('hidden');
                elements.extractedTujuanPreview.classList.add('hidden');
                elements.editVisualInput.classList.remove('hidden');
                elements.editCopywritingInput.classList.remove('hidden');
                elements.editTujuanInput.classList.remove('hidden');

                // Toggle tombol
                elements.editAnalysisBtn.classList.add('hidden');
                elements.saveAnalysisBtn.classList.remove('hidden');
                elements.cancelAnalysisBtn.classList.remove('hidden');

                // Nonaktifkan dropdown reel saat editing
                elements.reelSelect.disabled = true;

                elements.editVisualInput.focus();
            }

            function exitAnalysisEditMode() {
                // Sembunyikan textarea, tampilkan preview
                elements.editVisualInput.classList.add('hidden');
                elements.editCopywritingInput.classList.add('hidden');
                elements.editTujuanInput.classList.add('hidden');
                elements.extractedVisualPreview.classList.remove('hidden');
                elements.extractedCopywritingPreview.classList.remove('hidden');
                elements.extractedTujuanPreview.classList.remove('hidden');

                // Toggle tombol
                elements.editAnalysisBtn.classList.remove('hidden');
                elements.saveAnalysisBtn.classList.add('hidden');
                elements.cancelAnalysisBtn.classList.add('hidden');

                // Aktifkan kembali dropdown reel
                elements.reelSelect.disabled = false;
            }
            ...
            function saveAnalysisEdits() {
                ...
                const newVisual = elements.editVisualInput.value.trim();
                const newCopywriting = elements.editCopywritingInput.value.trim();
                const newTujuan = elements.editTujuanInput.value.trim();

                // Update state
                found.visual = newVisual || found.visual;
                found.copywriting = newCopywriting || '-';
                found.tujuan = newTujuan || '-';

                // Rebuild prompt dari gabungan visual + copywriting + tujuan
                const promptParts = [found.visual];
                if (found.copywriting && found.copywriting !== '-') {
                    promptParts.push(`with text overlay: "${found.copywriting}"`);
                }
                if (found.tujuan && found.tujuan !== '-') {
                    promptParts.push(`purpose: ${found.tujuan}`);
                }
                found.prompt = promptParts.join(', ');

                // Perbarui tampilan preview
                elements.extractedVisualPreview.textContent = found.visual;
                elements.extractedCopywritingPreview.textContent = found.copywriting;
                elements.extractedTujuanPreview.textContent = found.tujuan;

                exitAnalysisEditMode();
                showToast('Analisis Diperbarui', `Data Reel ${selectedVal} berhasil diperbarui.`, 'success');
            }

            // Event listeners edit/save/cancel
            elements.editAnalysisBtn?.addEventListener('click', enterAnalysisEditMode);
            elements.saveAnalysisBtn?.addEventListener('click', saveAnalysisEdits);
            elements.cancelAnalysisBtn?.addEventListener('click', exitAnalysisEditMode);
            ...
            function detectCountFromText(text, source = 'pdf_text') {
                ...
                // 1. Pola pencarian kata spesifik (Reel 1, Scene 2, dll) atau angka diikuti garis vertikal
                const explicitPatterns = [
                    /\b(?:reel|scene|ide|konten|adegan)\s*[-:#.]?\s*(\d{1,2})\b/g,
                    /\b(\d{1,2})\s*\|/g
                ];
                ...
                explicitPatterns.forEach((pattern) => {
                    for (const match of normalized.matchAll(pattern)) {
                        ...
                        if (val >= 1 && val <= 100) numbers.push(val);
                    }
                });

                // 2. Pemindaian Agresif: Mencari urutan angka (1, 2, 3, dst) yang berdiri sendiri
                // Sangat efektif untuk membaca format tabel PDF yang berantakan
                const sequencePattern = /(?:^|\n|\r)\s*(\d{1,2})\s*(?:\n|\r|\||\s[A-Za-z])/g;
                for (const match of normalized.matchAll(sequencePattern)) {
                    const val = Number.parseInt(match[1], 10);
                    if (val >= 1 && val <= 100) numbers.push(val);
                }
                ...
                // 3. Ambil angka unik dan urutkan
                const uniqueNumbers = [...new Set(numbers)].sort((a, b) => a - b);

                // 4. Cari urutan angka terjauh yang tidak terputus (dimulai dari 1)
                ...
                // Jika sistem menemukan urutan utuh (misal: 1, 2, 3, 4, 5), ambil angka tertingginya
                if (contiguousLast >= 2) return contiguousLast;

                // Fallback: kembalikan angka tertinggi yang ditemukan
            ...
            function updateStatusSummary() {
                ...
                // TAMBAHKAN PENGECEKAN KEAMANAN DI SINI
            ...
            async function requestRender() {
                ...
                const selectedReelVal = Number.parseInt(elements.reelSelect.value, 10) || 1;
                const currentReelData = state.processedReels.find(r => r.reel === selectedReelVal);
                if (currentReelData && currentReelData.prompt) {
                    formData.append('extracted_prompt', currentReelData.prompt);
                }
                ...
                try {
                    ...
                    // Jika server langsung mengembalikan hasil (misal via fallback Gemini)
                    ...
                    if (!data.prompt_ids || !Array.isArray(data.prompt_ids)) {
                        throw new Error('Format respons dari server tidak valid (prompt_ids hilang).');
                    }

                    const promptIdsString = data.prompt_ids.join(',');

                    // Polling ComfyUI status
                    const rawResults = await new Promise((resolve, reject) => {
                        const interval = setInterval(async () => {
                            try {
                                const statusRes = await fetch(`/carousel/status/${promptIdsString}`);
                                const statusData = await statusRes.json();

                                if (statusData.status === 'success' || statusData.status === 'done') {
                                    clearInterval(interval);
                                    resolve(statusData);
                                } else if (statusData.status === 'error' || statusData.status === 'failed') {
                                    clearInterval(interval);
                                    reject(new Error(statusData.message || 'Proses di AI gagal.'));
                                }
                            } catch (err) {
                                clearInterval(interval);
                                reject(new Error('Koneksi terputus saat mengecek status server.'));
                            }
                        }, 3000);
                    });

                    const results = normalizeRenderResults(rawResults).slice(0, 12);
            ...
            function showLoadingState() {
                ...
                // Tampilkan 1 skeleton saja karena carousel biasanya generate 1 batch/reel
                for (let i = 0; i < 1; i += 1) {
                ...
                return rawResults
                    .map((item, index) => {
                        if (typeof item === 'string') {
                            return {
                                title: `Scene Reel ${elements.reelSelect.value}`,
                                url: item,
                                status: 'Completed',
                                prompt: '',
                            };
