...
@section('content')
                ...
                <div class="relative p-6 border-b border-outline-variant/60 overflow-hidden">
                    ...
                    <div class="relative flex items-start justify-between gap-4">
                        <div>
                            ...
                            <p class="mt-2 text-body-md text-on-surface-variant">Unggah 2–3 gambar, beri arahan, lalu buat
                                variasi visual yang rapi.</p>
                        </div>
                        <div
                            ...
                            <span class="material-symbols-outlined"
                                style="font-variation-settings:'FILL' 1;">photo_library</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="p-6 space-y-7">
                    ...
                    <section>
                        <div class="flex items-center justify-between mb-3">
                            ...
                            <span id="imageCounterBadge"
                                class="px-3 py-1 rounded-full bg-surface-container-low text-on-surface-variant text-label-sm font-bold">0/3</span>
                        </div>

                        <input id="imageUploadInput" class="hidden" type="file" accept="image/*" multiple>

                        <div id="uploadDropZone"
                            class="upload-zone rounded-2xl border-2 border-dashed border-outline-variant bg-surface-container-low/70 p-4 transition-all cursor-pointer">
                        ...
                        <div class="mt-3 flex items-start gap-2">
                            ...
                            <p id="uploadHelperText" class="text-label-sm text-on-surface-variant">Belum ada gambar dipilih.
                                Unggah minimal 2 gambar.</p>
                        </div>
                    </section>
                    ...
                    <section>
                        ...
                        <div class="relative">
                            <textarea id="promptInput"
                                ...
                                placeholder="Contoh: gabungkan semua gambar menjadi satu komposisi cinematic, pencahayaan soft, warna harmonis, detail tajam, background bersih."></textarea>
                            <div
                                class="absolute bottom-3 right-3 px-2 py-1 rounded-full bg-surface-container-lowest text-[11px] text-on-surface-variant border border-outline-variant/60">
                                Opsional</div>
                        </div>
                    </section>
                    ...
                    <button id="generateButton"
                        ...
                        <span id="generateButtonIcon" class="material-symbols-outlined relative"
                            style="font-variation-settings:'FILL' 1;">auto_awesome</span>
                ...
                <div id="resultsEmptyState"
                    ...
                    <div
                        ...
                        <span id="emptyStateIcon"
                            class="material-symbols-outlined text-5xl text-primary">add_photo_alternate</span>
                    </div>
                    <h3 id="emptyStateTitle" class="relative font-headline-md text-on-surface mb-2">Belum ada hasil generasi
                    </h3>
                    <p id="emptyStateText" class="relative text-body-md text-on-surface-variant max-w-md leading-relaxed">
                        Unggah minimal 2 gambar, pilih rasio dan jumlah hasil, lalu klik tombol Buat Variasi.</p>

                    <div class="relative mt-6 flex flex-wrap justify-center gap-2">
                        <span
                            class="px-3 py-1.5 rounded-full bg-white/80 border border-outline-variant text-label-sm text-on-surface-variant">Min
                            2 gambar</span>
                        <span
                            class="px-3 py-1.5 rounded-full bg-white/80 border border-outline-variant text-label-sm text-on-surface-variant">Maks
                            3 gambar</span>
                        <span
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            const MAX_IMAGES = 3; // DIUBAH DARI 5 MENJADI 3
            const MIN_IMAGES = 2;
            ...
            async function requestGenerateImages() {
                ...
                if (!response.ok) throw new Error('Gagal mengirim perintah ke server.');

                const data = await response.json();
                if (!data.success) throw new Error(data.message || 'Gagal memulai proses generate.');

                if (!data.prompt_ids || !Array.isArray(data.prompt_ids)) {
                    throw new Error('Format response dari server tidak valid (prompt_ids hilang).');
                }
                ...
                return new Promise((resolve, reject) => {
                    const interval = setInterval(async () => {
                        try {
                            const statusRes = await fetch(`/gabung/status/${promptIdsString}`);
                            const statusData = await statusRes.json();

                            if (statusData.status === 'success' || statusData.status === 'done') {
                                clearInterval(interval);
                                resolve(normalizeGeneratedImages(statusData));
                            } else if (statusData.status === 'error' || statusData.status === 'failed') {
                                clearInterval(interval);
                                reject(new Error('Proses di AI gagal.'));
                            }
                        } catch (err) {
                            clearInterval(interval);
